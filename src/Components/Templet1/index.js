...
render(){
    const {groomDetails,date,size,font,color,timings,venue,formStatus,brideDetails}=this.state
    return (
        <div className='container'>
           ...
           <div className="templet">
                <div className="div">
                    <div>
                        <img className='rings' src="https://res.cloudinary.com/dkredoejm/image/upload/v1704618758/rings_hitdzv.png" width="100px" height="100px"/>
                    </div>
                    <h1 className='templeth1'>PLEASE JOIN US FOR AN ENGAGEMENT PARTY ON HONOR OF</h1>
                    <Heading type='text' font={font} size={size} color={color} angle={brideDetails.angle} onClick={this.updateFormStatus}>{brideDetails.name}</Heading>
                    <p>and</p>
                    <Heading type='text' font={font} size={size} color={color} angle={groomDetails.angle} onClick={this.updateFormStatus}>{groomDetails.name}</Heading>
                    <br/>
